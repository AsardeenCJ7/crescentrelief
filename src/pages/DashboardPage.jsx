...
import { 
  ...
  LayoutDashboard, History, Medal, Download, Copy, CheckCircle2, Bookmark, Clock, Share2, ShieldCheck, Star
} from "lucide-react";

// MOCK DATA
const ALL_DONATIONS = [
  { id: 1, campaign: "Gaza Emergency Relief", amount: 150, date: "Mar 20, 2024", status: "Completed", certId: "CERT-9821A" },
  { id: 4, campaign: "Earthquake Relief Fund", amount: 100, date: "Apr 02, 2024", status: "Pending", certId: null },
  { id: 2, campaign: "Clean Water for Yemen", amount: 50, date: "Feb 15, 2024", status: "Completed", certId: "CERT-9821B" },
  { id: 3, campaign: "Orphan Sponsorship", amount: 200, date: "Jan 10, 2024", status: "Completed", certId: "CERT-9821C" },
];

const FAVOURITES = [
  { id: 101, title: "Ramadan Food Parcels", target: 50000, raised: 35000, image: "https://images.unsplash.com/photo-1593113598332-cd288d649433?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80", category: "Food" },
  { id: 102, title: "Medical Supplies for Syria", target: 100000, raised: 75000, image: "https://images.unsplash.com/photo-1584515933487-779824d29309?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80", category: "Medical" },
];
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard icon={<Wallet className="text-primary" />} title="Total Donated" value={`$${user.totalDonated?.toLocaleString() || "0"}`} trend="+12% this month" color="primary" />
        <StatCard icon={<Heart className="text-rose-500" />} title="Campaigns Supported" value={user.campaignsSupported?.toString() || "0"} trend="3 active currently" color="rose" />
        <StatCard icon={<Award className="text-amber-500" />} title="Current Badge" value="Silver Donor" trend="75% to Gold" color="amber" />
        <StatCard icon={<Activity className="text-emerald-500" />} title="Lives Impacted" value="~1,250" trend="Estimated based on gifts" color="emerald" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-neutral-800 rounded-3xl p-6 shadow-sm border border-border-light dark:border-neutral-700">
          ...
          <div className="space-y-0">
            {ALL_DONATIONS.slice(0, 3).map((donation, idx) => (
              <div key={donation.id} className={`flex items-center justify-between py-4 ${idx !== 2 ? 'border-b border-border-light dark:border-neutral-700/50' : ''}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-semibold text-sm text-neutral-900 dark:text-white line-clamp-1">{donation.campaign}</h4>
                    <div className="text-xs text-neutral-500 dark:text-neutral-400">{donation.date}</div>
                  </div>
                </div>
        ...
        <div className="bg-gradient-to-br from-primary to-secondary rounded-3xl p-6 shadow-lg text-white relative overflow-hidden flex flex-col justify-center">
          ...
          <div className="relative z-10">
            ...
            <div className="bg-black/20 p-1 rounded-full flex items-center justify-between backdrop-blur-sm border border-white/10">
              <span className="text-xs font-mono ml-4 truncate">crescentrelief.org/ref/{user.id || '9182'}</span>
              <button 
                onClick={() => navigator.clipboard.writeText(`https://crescentrelief.org/ref/${user.id || '9182'}`)}
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="bg-white dark:bg-neutral-800 rounded-3xl shadow-sm border border-border-light dark:border-neutral-700 overflow-hidden">
      ...
      <div className="divide-y divide-border-light dark:divide-neutral-700">
        {ALL_DONATIONS.map((donation) => (
          <motion.div variants={itemVariants} key={donation.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-neutral-50 dark:hover:bg-neutral-700/30 transition-colors">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-neutral-900 dark:text-white text-lg">{donation.campaign}</h3>
                <div className="flex flex-wrap items-center gap-3 mt-1 text-sm text-neutral-500 dark:text-neutral-400">
                  <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> {donation.date}</span>
...
function BadgesTab({ user }) {
  ...
  const refLink = `https://crescentrelief.org/ref/${user.id || '9182'}`;
      ...
      <div className="bg-white dark:bg-neutral-800 rounded-3xl p-6 md:p-8 shadow-sm border border-border-light dark:border-neutral-700 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto">
          <div className="w-24 h-24 mx-auto bg-gradient-to-br from-neutral-200 to-neutral-300 dark:from-neutral-700 dark:to-neutral-600 rounded-full flex items-center justify-center shadow-inner mb-6 relative">
            ...
            <div className="absolute -bottom-2 bg-amber-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
              SILVER
            </div>
          </div>
          <h2 className="text-2xl font-heading font-bold text-neutral-900 dark:text-white mb-2">You are a Silver Donor!</h2>
          <p className="text-neutral-600 dark:text-neutral-400 mb-8">
            You're doing amazing! Get 5 more friends to donate via your link to unlock the <strong className="text-amber-500">Gold Badge</strong>.
          </p>
          
          <div className="mb-2 flex justify-between text-sm font-bold">
            <span className="text-neutral-500 dark:text-neutral-400">Silver</span>
      ...
      {FAVOURITES.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {FAVOURITES.map(campaign => (
            <motion.div variants={itemVariants} key={campaign.id} className="bg-white dark:bg-neutral-800 rounded-2xl overflow-hidden shadow-sm border border-border-light dark:border-neutral-700 hover:shadow-card-hover transition-all duration-300 group">
              ...
              <div className="p-5">
                ...
                <div className="w-full bg-neutral-100 dark:bg-neutral-700 rounded-full h-1.5 mb-2 overflow-hidden">
                  <div className="bg-primary h-1.5 rounded-full" style={{ width: `${(campaign.raised / campaign.target) * 100}%` }} />
                </div>
                <div className="flex justify-between text-xs font-medium mb-4">
                  ...
                  <span className="text-neutral-500 dark:text-neutral-400">${campaign.target.toLocaleString()} goal</span>
                </div>
                <Link to={`/campaigns/${campaign.id}`} className="block text-center w-full py-2.5 rounded-xl border border-primary text-primary font-bold text-sm hover:bg-primary hover:text-white transition-colors">
                  Donate Now
                </Link>
              </div>
            </motion.div>
